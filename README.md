...
You can 100% use the goerli testnet to see your NFTs rendered on opensea, but it's suggested that you test and build on a local development network so you don't have to wait as long for transactions.

### Running Scripts

The simple collectibles work on a local network,  however the advanced requires a testnet. We default to goerli since that seems to be the testing standard for NFT platforms. You will need testnet goerli ETH and testnet goerli LINK. You can find faucets for both in the [Chainlink documentation](https://docs.chain.link/docs/link-token-contracts#goerli).

# For the Simple ERC721

```
brownie run scripts/simple_collectible/deploy_simple.py --network goerli
brownie run scripts/simple_collectible/create_collectible.py --network goerli
```

# For the Advanced ERC721

You'll need [testnet goerli](https://faucet.goerli.io/) and [testnet LINK](https://goerli.chain.link/) in the wallet associated with your private key.

```
brownie run scripts/advanced_collectible/deploy_advanced.py --network goerli
brownie run scripts/advanced_collectible/create_collectible.py --network goerli
```

Then:

```
brownie run scripts/advanced_collectible/create_metadata.py --network goerli
brownie run scripts/advanced_collectible/set_tokenuri.py --network goerli
```
...
Add the file created in `metadata/goerli/NAME.json` to [IPFS](https://ipfs.io/) or [Pinata](https://pinata.cloud/).
